...
test_that("Annotation object can be initialized from a local GFF file (NCBI style)", {
  gff_file_path <- "fixtures/unit/ncbi_dummy.gff"
...
test_that("Annotation object can be initialized from a local GFF file (Ensembl style)", {
  ...
  expect_equal(
    tx2symbol[["ENST000001_1"]], "test_gene1",
    info = "tx2symbol mapping for ENST000001_1 failed"
  )
  expect_equal(
    tx2symbol[["ENST000002_2"]], "test_gene2",
    info = "tx2symbol mapping for ENST000002_2 failed"
  )

  # Test txid to gid translation
  tx2gid <- annotation_obj$generate_translate_dict(from = "txid", to = "gid")
  expect_equal(
    tx2gid[["ENST000001_1"]], "ENSG000001",
    info = "tx2gid mapping for ENST000001_1 failed"
  )
  expect_equal(
    tx2gid[["ENST000002_2"]], "ENSG000002",
    info = "tx2gid mapping for ENST000002_2 failed"
  )
  ...
  expect_setequal(
    unique(annotation_df_txid$gid), c("ENSG000001", "ENSG000002", "ENSG000003")
  )
  ...
  annotation_df_filtered <- annotation_obj_filtered$export_to_df(from = "txid")
  expect_equal(
    nrow(annotation_df_filtered), 15,
    info = "Filtered (.*transcript) did not result in 15 rows for txid export."
  )

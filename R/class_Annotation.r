...
Annotation <- R6::R6Class( # nolint
  ...
  private <- list(
    ...
    initialize_annotation = function(annotation, idmapping) {
      ...
      symbol_gid_df <- symbol_map_df[!is.na(symbol) & symbol != "" & !duplicated(symbol), .(symbol, gid)]
      ...
      symbol_uniprot_df <- symbol_map_df[!is.na(symbol) & symbol != "" & !is.na(uniprot) & !duplicated(symbol), .(symbol, uniprot)]
    ...
    initialize_from_directory = function(directory_path) {
      ...
      for (from_type in expected_from_types) {
        ...
        if (!file.exists(file_path)) {
          logging::logerror("Annotation$private$initialize_from_directory: File not found for type '%s': %s. Skipping.", from_type, file_path)
        }

        tryCatch({
          df <- data.table::fread(file_path, na.strings = "NA", stringsAsFactors = FALSE, data.table = FALSE)

          if (names(df)[1] != from_type) {
            logging::logerror("Annotation$private$initialize_from_directory: First column of %s is '%s', expected '%s'. Skipping file.",
                            file_path, names(df)[1], from_type)
            next
          }
          ...
          logging::logerror("Annotation$private$initialize_from_directory: Error reading or processing file %s for type '%s'. Error: %s",

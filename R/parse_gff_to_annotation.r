...
parse_gff_to_annotation <- function(
  ...
  if (!is.null(type_filter) && length(type_filter) > 0) {
    ...
    filtered_types <-
      unique_types[grepl(type_pattern, unique_types, perl = TRUE)]
  ...
  data.table::setnames(parsed_data, c("txid", "gid", "type", "symbol"))

  query <- paste0(unique(source2taxon), collapse = "%20OR%20tax_id%3D")
  url <- paste0(
